...
router.post("/", async(req,res)=>{
    ...
    if(!title || !author || !genre || !publication_date){
        return res.status(400).json({message:"Los campos titulo,autor y fecha son obligatorios"})
    }
...
router.patch("/:id",getBook,async(req,res)=>{
    if(!req.body.author && !req.body.genre && !req.body.publication_date && !req.body.title){
        res.status(404).json(
            {
